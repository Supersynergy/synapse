...
**v1.0 WORLD'S-BREAKTHROUGH RELEASE** — first open single-file format to fuse agent memory, BM25, vector, CRDT sync and signed distribution with **sub-20 ms median latency across every category** (50-usecase bench in [`bench/RESULTS-V1.md`](bench/RESULTS-V1.md)).
...
**v0.2 ships the new format family**: [`.synx` spec](docs/SYNX-FORMAT-V2.md) · [`.brainpack` v2](docs/BRAINPACK-V2.md) · [Why beyond SQLite](docs/WHY-BEYOND-SQLITE.md) · [Implementation plan](docs/SYNX-IMPLEMENTATION.md) · [RFC call](docs/RFC-CALL.md) · [Changelog](CHANGELOG.md)
...
```
┌──────── one file ─────────────┐
│  brain.synx                   │  ← default · Rust container · spec-stable
│  brain.brainpack              │     shippable wrapper · zstd · Ed25519 signed
│  brain.db                     │     legacy · SQLite engine · migrate-only
└───────────────────────────────┘
                │
┌──────────────▼────────────┐
│  synapsed daemon          │  ← 5.7 µs RPC · batch embed · BLAKE3 dedup
...
Memory you can `git commit`, sign with Ed25519, and sell as a pack.
...
| RPC round-trip | **5.7 µs** |
| Re-embed cached text (500 docs) | **1.24 ms** (1 273× repeat speedup) |
| `.brainpack` snapshot | **10 ms / ~1 MB** |
| Daemon cold-start | **~10 ms** |

### v0.2 — `.synx` container round-trip (10 000 docs)

| Op | Time | Throughput |
|---|---:|---:|
| Write 10k text chunks (zstd) | **478 ms** | 20 909 docs/s |
| Open container + manifest | **6.4 ms** | — |
| Read all 10k chunks + verify | **80 ms** | 125 000 docs/s |
| File size (10k short docs) | **1.67 MB** | 167 B/doc |
| `.brainpack` wrap of 1k docs | **~250 KB** | 4× over `.synx` bare |
...
### v0.2 — working with `.synx` and `.brainpack` directly

```rust
use synapse_core::{BrainPack, synx::{SynxWriter, SynxReader, ChunkKind, Codec}};
use synapse_core::synx::header::SynxFlags;

// 1. create a fresh .synx container
let mut w = SynxWriter::create("brain.synx", SynxFlags::COMPRESSED)?;
w.append(ChunkKind::TextBlob, Codec::Zstd, b"first memory")?;
w.finish()?;

// 2. ship it as a .brainpack (3-6× smaller, optionally signed)
BrainPack::pack("brain.synx", "release.brainpack")?;

// 3. on the consumer side
BrainPack::unpack("release.brainpack", "local.synx")?;
let r = SynxReader::open("local.synx")?;
println!("{} chunks, root hash {:02x?}", r.manifest.chunks.len(), r.footer.manifest_hash);
```

### Migrate v1 `.db` → v2 `.synx`

```bash
synapse migrate brain.db -o brain.synx      # one-way, keeps original
```
