...
_DEFAULT_SOCK = "/tmp/synapsedb.sock"


class SynapsePromptfooProvider:
    """
    Custom promptfoo provider that queries synapse and returns top-k passages.
    Invoke from promptfoo YAML: provider: python:synapsedb_promptfoo.provider:call
    """
    ...
    def _recv_n(s: socket.socket, n: int) -> bytes:
        ...
        while len(buf) < n:
            ...
            if not chunk:
                raise ConnectionError("synapsedb-daemon closed")
            buf += chunk
        return buf


# promptfoo entry point: python:synapsedb_promptfoo.provider:call
