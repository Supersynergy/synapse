...
use synapsedb_learn::LearnStore;
    ...
    match cli.cmd {
        ...
        Cmd::Shard { action } => match action {
            ...
            ShardCmd::Query {
                ...
                let q_arr: [f32; synapsedb_core::types::EMBED_DIM] = q_vec
                    .try_into()
        ...
        Cmd::Learn { action } => {
            ...
            match action {
                ...
                LearnCmd::Consolidate => {
                    let store = Store::open(&cli.file)?;
                    let report = synapsedb_learn::consolidate::run_consolidate(&store.conn)?;
                    println!(
                        "pairs_found={} merged={}",
                ...
                LearnCmd::Calibrate => {
                    let updated = synapsedb_learn::calibrate::update_calibration(&lstore)?;
        ...
        Cmd::Feedback {
            ...
            let learn_path = cli.file.with_extension("learn.db");
            let lstore = LearnStore::open(&learn_path)?;
            synapsedb_learn::feedback::record_accept(&lstore, &query_id, accepted_doc_id, &shard_id)?;
            println!(
...
fn print_hits(hits: &[synapsedb_core::Hit]) {
    for h in hits {
        println!(
            "{}\t{:.4}\t{}",
            h.id,
