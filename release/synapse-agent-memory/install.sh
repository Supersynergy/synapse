...
if [ -n "$version" ]; then
  case "$version" in
    synapse-memory-v*)
      ...
      ;;
    synapse-agent-memory-v*) version=${version#synapse-agent-memory-v} ;;
    v*) version=${version#v} ;;
  esac
fi

if [ "$legacy_release" = 1 ]; then
  release_stem="synapse-memory"
else
  release_stem="synapse-agent-memory"
fi
asset="${release_stem}-${target}.tar.gz"
if [ -n "${SYNAPSE_RELEASE_BASE:-}" ]; then
  base=${SYNAPSE_RELEASE_BASE%/}
elif [ -n "$version" ]; then
  base="$repo/releases/download/${release_stem}-v$version"
else
  base="$repo/releases/download/synapse-agent-memory-v$default_version"
fi
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/synapse-agent-memory-install.XXXXXX")
