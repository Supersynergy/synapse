...
$LegacyRelease = $Version -match "^synapse-memory-v"
$ReleaseStem = if ($LegacyRelease) { "synapse-memory" } else { "synapse-agent-memory" }
$Asset = "$ReleaseStem-$Target.zip"

if ($env:SYNAPSE_RELEASE_BASE) {
    $Base = $env:SYNAPSE_RELEASE_BASE.TrimEnd("/")
} elseif ($Version) {
    $CleanVersion = $Version -replace "^synapse-agent-memory-v", "" -replace "^synapse-memory-v", "" -replace "^v", ""
    $Base = "$Repo/releases/download/$ReleaseStem-v$CleanVersion"
} else {
    $Base = "$Repo/releases/download/synapse-agent-memory-v$DefaultVersion"
}
...
$Temp = Join-Path ([System.IO.Path]::GetTempPath()) ("synapse-agent-memory-" + [guid]::NewGuid())
...
try {
    ...
    $Source = Join-Path $Temp "$ReleaseStem-$Target\synx.exe"
    if (-not (Test-Path -LiteralPath $Source -PathType Leaf)) {
        throw "Archive does not contain $ReleaseStem-$Target\synx.exe"
    }
